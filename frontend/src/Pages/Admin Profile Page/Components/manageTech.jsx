...
function ManageTech() {
  ...
  const handleDeactivateTech = async (techId) => {
    try {
      const response = await axios.put(
        `https://library-database-backend.onrender.com/api/technology/deactivateTechnology/${techId}`
      );
      setStatusMessage(response.data.success ? 'Technology deactivated successfully!' : `Failed to deactivate technology: ${response.data.message || 'Unknown error'}`);
      fetchAllTech();
    } catch (error) {
      setStatusMessage(`Error deactivating technology: ${error.message}`);
    }
  };
              ...
              <tr key={tech.techId} style={{ borderBottom: '1px solid #ddd', height: '60px' }}>
                ...
                <td style={{ padding: '10px', display: 'flex', gap: '5px', justifyContent: 'center' }}>
                  ...
                  <button style={{
                    ...
                    padding: '8px 15px',
                    cursor: 'pointer'
                  }} onClick={() => handleDeactivateTech(tech.techId)}>Deactivate</button>
                </td>
              </tr>
            ))}
          </tbody>
