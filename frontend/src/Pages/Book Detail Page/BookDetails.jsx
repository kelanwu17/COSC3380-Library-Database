...
function BookDetails() {
  ...
  async function waitListBook(e) {
    ...
    try {
      ...
      const response = await axios.post(
        "https://library-database-backend.onrender.com/api/waitlist/createWaitlist",
        waitListData
      );
  ...
  async function cancelwaitListBook(e) {
    ...
    try {
      const response = await axios.put(
        `https://library-database-backend.onrender.com/api/waitlist/cancelWaitlist/${waitListID}`
      );
      console.log(response);
  ...
  async function checkout(e) {
    ...
    try {
      ...
      const response = await axios.post(
        "https://library-database-backend.onrender.com/api/checkoutbook/insertCheckOutBook",
        dataToSend
      );
  ...
  async function reserveItem(e) {
    ...
    try {
      ...
      const response = await axios.post(
        "https://library-database-backend.onrender.com/api/reserve/createReserve",
        reserveData
      );
      setReserve(true);
  ...
  async function cancelReserveItem(e) {
    ...
    try {
      const response = await axios.put(
        `https://library-database-backend.onrender.com/api/reserve/cancelReserve/${reserveID}`
      );
      setReserve(false);
  ...
  useEffect(() => {
    ...
    const fetchBookDetails = async () => {
      try {
        const response = await axios.get(
          `https://library-database-backend.onrender.com/api/books/${id}`
        );
    ...
    const fetchInstance = async () => {
      try {
        const response = await axios.get(
          `https://library-database-backend.onrender.com/api/bookInstance/${id}`
        );
        ...
        if (!reserve) {
          const availableInstance = instances.find(
            (instance) => instance.checkedOutStatus == 0
          );
    ...
    const fetchWaitList = async () => {
      try {
        const response = await axios.get(
          `https://library-database-backend.onrender.com/api/waitlist/${userId}`
        );
        const memberHistory = response.data;
    ...
    const fetchReserveList = async () => {
      try {
        const response = await axios.get(
          `https://library-database-backend.onrender.com/api/reserve/${userId}`
        );
        const memberHistory = response.data;
        const instanceFound = memberHistory.find(
          (instance) =>
  ...
  const filteredSimilarBooks = similarBooks.filter(
  ...
  return (
    <div>
      ...
      <div className="h-8">
        <button
          className="ml-4 mt-4 h-6 border bg-amber-900 w-32 rounded-lg text-white text-bold border-black"
        ...
        <div className="ml-2 mt-2 flex flex-col">
          ...
          <p className="text-sm mt-1">
            ...
            {genres.map((g, index) => (
              <span key={index}>
                <Link to={`/books/${g}`}>
                  <span className="text-blue-600 hover:text-purple-800">
                    {g}
                  </span>
                </Link>
